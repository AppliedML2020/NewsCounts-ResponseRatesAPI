...
```{r}
image_url <- "https://newscounts.appspot.com/api/response_rates/state?FORMAT=PNG&DATA=CRRALL&STATE=NY,PA,AK,CA"
```
...
|```STATE```| Required if ```FORMAT``` is PNG | ISO 2 letter abbreviation for U.S. states. If ```FORMAT``` is PNG you can select many states separated by a comma.  | NY |
...
|```COUNTY```| Required if ```FORMAT``` is PNG | [FIPS county codes](https://www.census.gov/prod/techdoc/cbp/cbp95/st-cnty.pdf). If ```FORMAT``` is PNG you can select many counties separated by a comma. | 1 (For Autauga County in Alabama) |
...
|```TRACT``` | Required if ```FORMAT``` is PNG | 2020 Census tract number. If ```FORMAT``` is PNG you can select many tracts separated by a comma. | 020100|
